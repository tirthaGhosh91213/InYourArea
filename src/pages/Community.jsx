...
export default function Community() {
  ...
  const fetchPosts = async () => {
    try {
      setLoading(true);
      const res = await axios.get("https://miami-only-great-buf.trycloudflare.com/api/v1/community", {
  ...
  const fetchComments = async (postId) => {
    try {
      const res = await axios.get(
        `https://miami-only-great-buf.trycloudflare.com/api/v1/comments/community-posts/${postId}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      if (res.data.success) {
