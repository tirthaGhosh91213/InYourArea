...
export default function CreateLocalNewsPost() {
  ...
  useEffect(() => {
    const fetchExistingDistricts = async () => {
      try {
        const res = await axios.get("https://api.jharkhandbiharupdate/api/v1/districts", {
  ...
  const ensureDistrictExists = async (districtName) => {
    ...
    try {
      const res = await axios.post(
        "https://api.jharkhandbiharupdate/api/v1/districts",
        { name: districtName },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      if (res.data.success) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(
        "https://api.jharkhandbiharupdate/api/v1/district-news",
        form,
