// src/pages/CreateLocalNewsPost.jsx
...
export default function CreateLocalNewsPost() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.data.success) {
        toast.success("Local News posted successfully!");
        navigate("/localnews");
      }
    } catch (err) {
      toast.error("Failed to post Local News");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <motion.div className="min-h-screen bg-gradient-to-b from-green-50 to-white flex flex-col items-center py-10 px-6">
      ...
      <motion.div className="w-full max-w-lg bg-white p-8 rounded-2xl shadow-2xl border border-gray-100">
        <h1 className="text-3xl font-bold text-green-600 text-center mb-6">Create Local News Post</h1>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} type="submit" className="w-full bg-gradient-to-r from-green-400 to-green-500 text-white py-3 rounded-full shadow-lg">
            {loading ? "Posting..." : "Post Local News"}
          </motion.button>
        </form>
      </motion.div>
    </motion.div>
