...
export default function AdminDashboard() {
  ...
  const handleAction = async (id, action) => {
    try {
      ...
      setPopupMessage(
        `${activeTab.toUpperCase()} ${action === "approve" ? "Approved ✅" : "Rejected ❌"}`
      );
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-green-50 text-gray-800 p-4 sm:p-8">
            ...
            <AnimatePresence>
              {items.map((item, index) => (
                <motion.div
                  ...
                  className={`cursor-pointer grid grid-cols-1 sm:grid-cols-3 items-center px-4 py-4 sm:px-6 sm:py-3 gap-2 sm:gap-0 rounded-xl shadow-md border transition-all duration-300 ${
                    index % 2 === 0
                ...
                >
                  ...
                  {/* Created Date */}
                  <div className="flex items-center gap-2 text-gray-700 justify-start sm:justify-center">
                    <Calendar size={16} />
                    <span>{new Date(item.createdAt).toLocaleDateString()}</span>
                  </div>

                  {/* Author & Actions */}
                  <div className="flex flex-col sm:flex-row sm:justify-center items-start sm:items-center gap-2 sm:gap-3">
                    <div className="flex items-center gap-1 text-green-800 font-medium">
                      <User size={16} /> {item.author?.firstName} {item.author?.lastName || ""}
                    </div>
                    <div
                    ...
                    >
                      <motion.button
                        onClick={() => setConfirmAction({ id: item.id, action: "approve" })}
