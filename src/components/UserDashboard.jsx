...
      console.log("Fetched data:", res.data.data); // Debug log
      
      setModalContent(res.data.data);
      setModalType(type);
      setShowFullContent(false); // Reset to collapsed state
    } catch (error) {
...
  // Open edit modal with current news data (WITHOUT districtName)
...
  // Submit update with success popup
...
{activeTab !== "comments" && (
                  <div className="flex gap-2 mt-2">
                    {/* Show View Details button for everything EXCEPT localNews */}
                    {activeTab !== "localNews" && (
                      <button
...
                  {modalType === "localNews" ? "Content" : modalType === "community" ? "Description" : "Description"}
                </h3>
...
                    <span className={`px-2 py-0.5 rounded text-xs ${getStatusColor(modalContent.status)}`}>
                      {modalContent.status}
                    </span>
                  </p>
                </div>
...
              {/* Images with lightbox functionality - NO HOVER OVERLAY */}
...
      {/* Modal for Edit Local News (WITHOUT district name) */}
      <AnimatePresence>
        {editModalOpen && (
          <motion.div
            initial={{ opacity: 0 }}
